...
export default function AddHabitPage(){
    ...
    async function handleNewHabit(e: React.FormEvent){
        ...
        const { data, error } = await supabase.from('habits').insert([{user_id: user.id, name:habitName }]).select()
        if(error) {
            setError(error.message)
        } else {
            setHabitId(data[0].id) //use the new habit id to potentially update habit with motivation data[0].id 
            setStep(2)
        }
    }
    async function handleMotivation(e: React.FormEvent){
        ...
        const { data: { user } } = await supabase.auth.getUser()
        if (!user) { router.push('/login'); return; }
        const { data, error } = await supabase.from('habits').update({motivation: motivation}).eq('id', habitId)
        if(error) {
            setError(error.message)
        } else {
    ...
    const isValidHabit = habitName.trim().length > 0;
        ...
        <div className="min-h-screen bg-gray-950 flex items-center justify-center p-4">
            <div className="w-full max-w-sm">
                <div className="text-center mb-8">
                    ...
                    <p className="text-slate-400 mt-2 text-sm">A journey of million steps starts with the 1st</p>
                </div>
            {step === 2 && (
